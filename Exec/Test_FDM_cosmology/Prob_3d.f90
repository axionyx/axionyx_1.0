...
!just the axion star.
      ...
      double precision x,y,z,xtilde,ytilde,ztilde

      hubl = 0.7d0
      meandens = 2.775d11 * comoving_h**2* comoving_OmM !background density
      
      ! print *, "meandens fdm = ",meandens

      ! rc = 1.3d0 * 0.012513007848917703d0 / (dsqrt(m_tt * hubl) * comoving_OmAx**(0.25d0))
      rc = 0.125d0*(domhi(1)-domlo(1))*delta(1)/sqrt(2.d0)*4.0d0
      ...
      do k = lo(3), hi(3)
         do j = lo(2), hi(2)
            do i = lo(1), hi(1)
               ...
               xtilde = 8.0d0*M_PI*x/ (domhi(1)-domlo(1)) / delta(1)
               ytilde = 8.0d0*M_PI*y/ (domhi(2)-domlo(2)) / delta(2)
               ztilde = 8.0d0*M_PI*z/ (domhi(3)-domlo(3)) / delta(3)
               ...
               ! axion(i,j,k,UAXDENS) = axion(i,j,k,UAXDENS)*(1.d0-del*cos(xtilde))*(1.d0-del*cos(ytilde))*(1.d0-del*cos(ztilde))
               axion(i,j,k,UAXPHAS) = 0.0 !20.0*r-10.0
               ! axion(i,j,k,UAXDENS) = ratio_fdm*meandens                                                                                            
               axion(i,j,k,UAXRE)   = dsqrt(axion(i,j,k,UAXDENS))!*cos(axion(i,j,k,UAXPHAS))                                                                                                                  
               axion(i,j,k,UAXIM)   = 0.0d0!sqrt(axion(i,j,k,UAXDENS))*sin(axion(i,j,k,UAXPHAS))                                                                                                                  
