...
!           print *, "WARNING: functional determinant is changing very quickly! Arguments are: ",Arg1, Arg2, Cpqtemp, Cpq, particles(n)%id, particles(n)%cpu
           particles(n)%mass = 1.0 !0.8*dt
!        else
!           particles(n)%mass = 0.0
        endif
...
!         endif
  ...
  subroutine update_fdm_particles_phase(np, particles, accel, accel_lo, accel_hi, &
                                      plo, dx, dt, a_prev, a_cur, do_move) &
       bind(c,name='update_fdm_particles_phase')

    use iso_c_binding
    use amrex_error_module
    use amrex_fort_module, only : amrex_real
    use particle_mod      , only: fdm_particle_phase_t

    integer,              intent(in   )        :: np
    type(fdm_particle_phase_t), intent(inout)    :: particles(np)
    integer,              intent(in   )        :: accel_lo(3), accel_hi(3)
    real(amrex_real),     intent(in   )        :: accel &
         (accel_lo(1):accel_hi(1),accel_lo(2):accel_hi(2),accel_lo(3):accel_hi(3),3)
    real(amrex_real),     intent(in   )        :: plo(3),dx(3),dt,a_prev,a_cur
  ...
  end subroutine update_fdm_particles_phase

  subroutine update_gaussian_beams_phase(np, particles, accel, accel_lo, accel_hi, &
                                       phi, phi_lo, phi_hi, &
                                       plo, dx, dt, a_prev, a_cur, do_move) &
       bind(c,name='update_gaussian_beams_phase')
  ...
  end subroutine update_gaussian_beams_phase
