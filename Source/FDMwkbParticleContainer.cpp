...
void                                                                                                                                                                                                            
FDMwkbParticleContainer::DepositFDMParticles(MultiFab& mf_real, MultiFab& mf_imag, int lev, amrex::Real a) const
{
...
#pragma omp parallel
#endif
  {
    ...
    for (MyConstParIter pti(*this, lev); pti.isValid(); ++pti) {
      ...
      deposit_fdm_particles_wkb(particles.data(), &np, data_ptr_real,
...
{
    BL_PROFILE("FDMwkbParticleContainer::InitCosmo1ppcMultiLevel()");
	            ...
	            if (!this->Where(p, pld))
                    {
      		        this->PeriodicShift(p);
                        ...
                        if (!this->Where(p, pld))
                            amrex::Abort("FDMwkbParticleContainer::InitCosmo1ppcMultiLevel():invalid particle");
                    }
...
void
FDMwkbParticleContainer::InitGaussianBeams (long num_particle_fdm, int lev, int nlevs, const Real hbaroverm, const Real sigma_ax, const Real gamma_ax)
{
  const int       MyProc      = ParallelDescriptor::MyProc();
  const Geometry& geom        = m_gdb->Geom(lev);
  const Real*     dx          = geom.CellSize();
